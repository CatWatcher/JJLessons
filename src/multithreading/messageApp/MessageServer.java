package multithreading.messageApp;

import java.io.IOException;
import java.net.ServerSocket;
import java.net.Socket;
import java.util.Set;
// коллекции для работы с потоками
import java.util.concurrent.BlockingDeque;
import java.util.concurrent.CopyOnWriteArraySet;
import java.util.concurrent.LinkedBlockingDeque;

public class MessageServer {

    private int port;

    // тут хранятся соединения
    // чтобы можно было разослать всем
    private Set<Connection> connections = new CopyOnWriteArraySet<>();

    // блокирующая очередь
    private BlockingDeque<Message> messages = new LinkedBlockingDeque<>();


    public MessageServer(int port) {
        this.port = port;
    }


    public static void main(String[] args) {
        MessageServer server = new MessageServer(8090);
        server.start();
    }

    // читающий поток для каждого соединения
    public void start() {
        new Thread (new Writer()).start();

        try (ServerSocket serverSocket = new ServerSocket(port)){
            System.out.println("server started...");

            while (true) {
                // создаем соединение для каждого нового клиента
                // через сервер сокет (новый сокет для каждого)
                Socket socket = serverSocket.accept();
                Connection connection = new Connection(socket);
                // добавляем соединение в коллекцию
                // чтобы работать с ним в дальнейшем
                connections.add(connection);

                new Thread(new Reader(connection)).start();
            }

        } catch (IOException e) {
            e.printStackTrace();
        }
    }


    private  class Writer implements Runnable {
        @Override
        public void run() {
            try {
                while (!Thread.currentThread().isInterrupted()) {
                    // получаем сообщения из очереди
                    Message message = messages.take();
                    for (Connection connection : connections) {
                        // отправляем сообщение
                        connection.sendMessage(message);
                    }
                    // удаляем из очереди
                    connections.remove(message);

                }
            } catch (InterruptedException | IOException e) {
                e.printStackTrace();
            }

        }
    }

    private class Reader implements Runnable {

        private Connection connection;

        public Reader(Connection connection) {
            this.connection = connection;
        }

        @Override
        public void run() {
            try {
                while (!Thread.currentThread().isInterrupted()) {
                    Message message = connection.readMessage();
                    messages.add(message);
                    System.out.println(message);
                }
            } catch (ClassNotFoundException | IOException e) {
                e.printStackTrace();
            }
        }
    }
}
